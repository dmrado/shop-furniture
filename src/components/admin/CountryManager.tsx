...
import { createCountry, getAllCountries, removeCountry, updateCountry } from '@/actions/dictionaryActions'
...
const CountryManager = ({ initialCountries, itemsPerPage, currentPage, totalCount }: CountryManagementClientProps) => {
    ...
    const handleEditClick = (country: DictionaryItem) => {
        ...
        setCurrentCountry({ id: country.id, name: country.name, description: desc, isActive: country.isActive ?? true })
                    ...
                    countries.map((country) => (
                        <div key={country.id} className={cardStyle}>
                            <div className="relative flex flex-col items-center flex-grow">
                                ...
                                <span className={nameStyle} title={country.name}>{country.name}</span>
                                <span className={descriptionStyle} title={country.description || ''}>{country.description}</span>
                            </div>
                            <div className={actionsContainerStyle}>
                                <button onClick={() => handleEditClick(country)} className={actionButtonStyle}>
                                    <PencilIcon className="h-4 w-4 mr-1"/> Редактировать
                                </button>
                                <button onClick={() => handleDeleteClick(country)} className={deleteButtonStyle}>
            ...
            {showModal && (
                <Modal onClose={() => { setShowModal(false); setDescriptionCharCount(0); }}>
                    ...
                    <form action={handleSubmit} className="space-y-4">
                        {currentCountry?.id && (<input type="hidden" name="id" value={currentCountry.id}/>)}
                        <div>
                        ...
                        <div>
                            <label htmlFor="countryDescription" className="block text-sm font-medium text-gray-700">Описание
