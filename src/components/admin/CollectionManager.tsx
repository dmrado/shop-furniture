...
import { createCollection, getAllCollections, removeCollection, updateCollection } from '@/actions/dictionaryActions'
import Modal from '@/components/ui/Modal'
import { usePathname, useRouter, useSearchParams } from 'next/navigation'
import { PencilIcon, TrashIcon } from '@heroicons/react/24/outline'
import ReactPaginateWrapper from '@/components/site/ReactPaginateWrapper'

type CollectionManagementClientProps = {
...
const CollectionManager = ({ initialCollections, itemsPerPage, currentPage, totalCount }: CollectionManagementClientProps) => {
    ...
    const handleEditClick = (collection: DictionaryItem) => {
        const desc = collection.description || ''
        setCurrentCollection({ id: collection.id, name: collection.name, description: desc, isActive: collection.isActive ?? true })
    ...
    const handleDeleteClick = (collection: DictionaryItem) => {
    ...
    return (
        <div className="p-6 bg-white rounded-lg shadow-md">
            <h2 className="text-2xl font-bold mb-4">Управление коллекциями</h2>
                    ...
                    collections.map((collection) => (
                        <div key={collection.id} className={cardStyle}>
                            <div className="relative flex flex-col items-center flex-grow">
                                ...
                                <span className={nameStyle} title={collection.name}>{collection.name}</span>
                                <span className={descriptionStyle} title={collection.description || ''}>{collection.description}</span>
                            </div>
                            <div className={actionsContainerStyle}>
                                <button onClick={() => handleEditClick(collection)} className={actionButtonStyle}>
                                    <PencilIcon className="h-4 w-4 mr-1"/> Редактировать
                                </button>
                                <button onClick={() => handleDeleteClick(collection)} className={deleteButtonStyle}>
            ...
            {showModal && (
                <Modal onClose={() => { setShowModal(false); setDescriptionCharCount(0); }}>
                    ...
                    <form action={handleSubmit} className="space-y-4">
                        {currentCollection?.id && (<input type="hidden" name="id" value={currentCollection.id}/>)}
                        <div>
                        ...
                        <div>
                            <label htmlFor="collectionDescription" className="block text-sm font-medium text-gray-700">Описание
