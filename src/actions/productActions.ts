...
export const putProductToCartAction = async (productId: number) => {
  ...
  const newCartRow = {
    discount: newProductInCart.discount,
    id: newProductInCart.id,
    quantity: newProductInCart.quantity,
    productId: newProductInCart.productId,
    userId: newProductInCart.userId,
    updatedAt: newProductInCart.updatedAt,
    createdAt: newProductInCart.createdAt
  }

console.log(">>>>>>>>>>>>>>>>>>>> >>> >>> newCartRow", newCartRow)
